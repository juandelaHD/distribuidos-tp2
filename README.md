...
El Joiner solo reenvía los tops parciales preservando el `client_id` y descarta las marcas de EOF, que en este escenario no aportan información útil al gateway porque aún no hay coordinación de varios Aggregators por cliente.
...
Antes de llegar a la solución definitiva evalué tres enfoques. Los tres coinciden en que el Aggregator necesita contar `N` EOFs por cliente antes de finalizar (una barrera embebida parametrizada por `SUM_AMOUNT`), pero difieren en cómo garantizar que esos `N` EOFs lleven datos completos.
...
Lo descarté por dos motivos. Primero, rompe la abstracción del middleware ya que compartir el canal AMQP entre consumidores obliga a que Sum conozca detalles de conexión que el middleware está pensado para ocultar. Segundo por temas de performance. `Qos(1, global=true)` serializa a los workers y elimina el paralelismo interno que RabbitMQ habilita con un prefetch alto.
...
Un detalle del Sum es que al sumar el consumer de `sum_ring_i`, el nodo pasa a tener dos consumers concurrentes que tocan los mismos maps (`fruitItemMaps`, `rowCounts`, `expectedTotals`). Lo resolví con un fan-in event loop,donde los consumers solo encolan eventos a un channel bufferado y un único goroutine los procesa. No es una decisión de performance, ambos enfoques serializan el mismo acceso al estado y el paralelismo real del sistema lo provee tener `N` Sums, no la concurrencia interna. El ACK de cada mensaje se dispara dentro del handler del event loop, no al encolar, así que los mensajes en vuelo entre el consumer y el loop siguen unacked en el broker. Esto mantiene que un mensaje se reconoce solo cuando efectivamente se procesó y a la vez se presentan dos capas de "backpressure" (este buffer y el prefetch de AMQP). La capacidad `256` la medí al realizar pruebas con datasets de 30.000 filas, `buffer=64` saturó dos de los tres Sums y el pico con `buffer=1024` rondó los 100 mensajes, así que `256` deja un margen cómodo sin desperdiciar memoria pero podría ajustarse según la cantidad de "productores" que alimentan el channel del Sum y el tiempo de procesamiento por evento en el loop.
...
El top global es `[manzana=100, frutilla=90, banana=80]`. Los tops parciales que los Aggregators mandan al Joiner son `[manzana, banana, pera]`, `[frutilla, frambuesa]` y `[naranja, mandarina]`. Las tres frutas del top global aparecen en esos parciales — no podía ser de otra forma, porque cada una es, como mínimo, top 3 de su propio shard. Al Joiner le alcanza con unir los parciales, ordenar y cortar en `TOP_SIZE` para reconstruir el top global. Deja de ser un simple forwarding y pasa a ser una barrera que cuenta `K` EOFs por cliente antes de emitir el resultado final.

Para el canal entre el Sum y el Aggregator decidí usar directamente `K` colas nombradas `aggregation_{i}`, una por Aggregator. Cada Sum arma una instancia por cola destino y publica a la que corresponde por hash; el Aggregator `i` simplemente consume de la suya. El Joiner, por su parte, se mantuvo consumiendo de una cola común donde los `K` Aggregators publican sus tops parciales.

En cuanto a costos, la distribución es equitativa en expectativa cuando la cantidad de frutas distintas es bastante mayor que `K`, porque el Sum consolida por fruta antes de flushear y el volumen hacia cada Aggregator queda acotado por las *frutas distintas en su shard*. El peor caso aparece cuando la cantidad de frutas distintas es menor o igual que `K` porque algunos Aggregators quedan ociosos recibiendo solo EOFs. Es un límite estructural del sharding por clave — no existe función que reparta en más baldes que claves disponibles sin romper la invariante "la misma fruta siempre cae en el mismo Aggregator" de la que depende la correctitud del join final. A nivel de mensajes de coordinación, cada cliente genera `N × K` EOFs broadcast del Sum al Aggregator, más los datos (acotados por las frutas distintas del cliente) y `K` tops parciales hacia el Joiner; lineal en ambas dimensiones.

### Parte 4 — Graceful shutdown

Agregué soporte de `SIGINT`/`SIGTERM` en los tres nodos que nosotros controlamos (Sum, Aggregator, Joiner), siguiendo el mismo patrón que el gateway: una goroutine dedicada que bloquea en un `signal.Notify` y, al recibir la señal, dispara un método `shutdown()` idempotente.

Me parece importante destacar el orden de cierre. Los consumidores se bloquean adentro de `StartConsuming`, que itera sobre el canal de deliveries de AMQP, así que para desbloquearlos hay que cerrar ese canal, y eso lo hace `StopConsuming`. Pero los callbacks que ejecuta cada consumidor pueden estar publicando sobre otros middlewares productores (el Sum, por ejemplo, consume del input y publica al ring y a las colas de Aggregator), así que cerrar los productores antes de tiempo haría fallar a un callback en vuelo. De esta manera primero dejar de consumir sobre cada consumidor, después un `WaitGroup.Wait()` que garantiza que todas las goroutines de consumo terminaron de ejecutar su último callback, y recién después un `Close()` sobre cada instancia de middleware, tanto de consumidores y productores, para liberar canales y conexiones TCP.

Concretamente, el Sum tiene dos consumidores (`input_queue` y su cola del anillo) más `K + 1` productores (las `K` colas de Aggregator y el siguiente del anillo), así que el WaitGroup espera a los dos consumidores antes de cerrar los productores. Si `StartConsuming` retorna un error **mientras `running` sigue en true** (sin que nadie haya pedido el shutdown), el propio consumidor dispara el shutdown para que el proceso no quede colgado con las otras goroutines vivas.
