...
Me parece importante destacar el orden de cierre. Los consumidores se bloquean adentro de `StartConsuming`, que itera sobre el canal de deliveries de AMQP asi que para desbloquearlos hay que cerrar ese canal, y eso lo hace `StopConsuming`. Pero los callbacks que ejecuta cada consumidor pueden estar publicando sobre otros middlewares productores (el Sum, por ejemplo, consume del input y publica al ring y a las colas de Aggregator), así que cerrar los productores antes de tiempo haría fallar a un callback en vuelo. De esta manera primero dejar de consumir sobre cada consumidor, después un `WaitGroup.Wait()` que garantiza que todas las goroutines de consumo terminaron de ejecutar su último callback, y recién después un `Close()` sobre cada instancia de middleware, tanto de consumidores y productores, para liberar canales y conexiones TCP. Cualquier mensaje que hubiera quedado en el buffer de deliveries sin ackear se reencola automáticamente por el `autoAck=false` configurado, por ende no es necesario drenar esos buffers manualmente antes de cerrar los productores.

Concretamente, el Sum tiene dos consumidores (`input_queue` y su cola del anillo) más `K + 1` productores (las `K` colas de Aggregator y el siguiente del anillo) asi que el WaitGroup espera a los dos consumidores antes de cerrar los productores. El Aggregator y el Joiner tienen un consumidor y un productor, así que es el mismo patrón pero más acotado. Si `StartConsuming` retorna un error **mientras `running` sigue en true** (sin que nadie haya pedido el shutdown), el propio consumidor dispara el shutdown para que el proceso no quede colgado con las otras goroutines vivas.
